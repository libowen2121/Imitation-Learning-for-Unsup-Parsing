'''
data preprocessing for snli dataset including labels
'''
import os
import re
import cPickle
...
class Corpus(object):
    def __init__(self, path, punctuations=False):
        ...
        print 'tokenizing...'
        self.train, self.train_sens, self.train_trees, self.train_labels = self.tokenize(train_file)
        self.valid, self.valid_sens, self.valid_trees, self.valid_labels = self.tokenize(valid_file)
        self.test, self.test_sens, self.test_trees, self.test_labels = self.tokenize(test_file)
