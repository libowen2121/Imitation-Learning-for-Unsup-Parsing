...
rest_file_ids = []
for id in file_ids:
    if 'WSJ/00/WSJ_0000.MRG' <= id <= 'WSJ/21/WSJ_2199.MRG':
        train_file_ids.append(id)   
            # Note that: for unsupervised parsing training, training set can be all the dataset. Find details in the original paper.
...
class Corpus(object):
    def __init__(self, path):
        dict_file_name = os.path.join(path, 'prpn_multinli_dict.pkl')   # 
