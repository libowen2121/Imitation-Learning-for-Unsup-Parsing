...
def train_accumulate(model, data_manager, A, batch):
    #TODO: the original command has no effect
...
def stats(model, optimizer, A, step, log_entry):
    ...
    log_entry.sbs_loss = A.get_avg('sbs_loss')
    log_entry.sbs_accuracy = A.get_avg('sbs_acc')         
...
def eval_accumulate(model, data_manager, A, batch):
    ...
    # the original line does not have an effect
    #X_batch, transitions_batch, y_batch, num_transitions_batch, train_ids, _ = batch
...
def train_format(log_entry, extra=False, rl=False):
    ...
    # Accuracy Component.
    stats_str += " Acc: classfify {class_acc:.5f} tr(invalid) {transition_acc:.1f} sbs {sbs_acc: .5f}"

    # Cost Component.
    stats_str += " Loss: total {total_loss:.3f} xent {xent_loss:.3f} tr {transition_loss:.1f} reg {l2_loss:.3f} sbs {sbs_loss: .3f}"
...
def eval_format(evaluation, extra=False):
    eval_str = "Step: {step} Eval acc: cl {class_acc:.5f} f1: {f1:.3f} tr {transition_acc:.1f} {filename} Time: {time:.5f}"
...
def log_formatter(log_entry, extra=False, rl=False):
    ...
    args = {
        ...
        'sbs_acc': log_entry.sbs_accuracy,
